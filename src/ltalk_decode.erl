...
fliter_cmd(Line,{unknowncmd,Cmd0}=Cmd) ->
	#line{cmd=Cmd0,data=Line#line.data,state=?ERROR_CODE_CMD_UNKNOWN};

fliter_cmd(Line,undefined) ->
	Line;
...
fliter_cmd_with_undefined_cmd_test() ->
	?assertEqual(#line{cmd="loginlogin",data="   jias",state=?ERROR_CODE_CMD_UNKNOWN},getLine(" -loginlogin   jias")).
fliter_cmd_with_cmd_flag_test() ->
	?assertEqual(#line{cmd="",data=" ",state=?ERROR_CODE_CMD_UNKNOWN},getLine(" - ")).
